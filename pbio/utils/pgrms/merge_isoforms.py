...
def merge_gene_group(g, transcript_id_field, transcript_id_format_str):
    ...
    if gene_id is not None:
        attributes.extend(["gene_id " + '"' + gene_id + '"'])

    if gene_name is not None:
        attributes.extend(["gene_name" + '"' + gene_name + '"'])
...
def main():
    ...
    parser.add_argument(
        "--group-attribute",
        help="The attribute by which the features " "will be merged",
    ...
    merged_genes_df = bed_utils.sort(merged_genes_df, seqname_order=args.chr_name_file)
