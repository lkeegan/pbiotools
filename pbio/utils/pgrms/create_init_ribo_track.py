...
def main():
    ...
    parser.add_argument(
        "outloc",
        help="The output directory. Temporary files are also"
        "written to this location if converting to BigWig.",
    )

    parser.add_argument(
        "--use-pretty-names",
        help="If this flag is given, then will use the names"
        "in 'riboseq_sample_name_map' if present.",
        action="store_true",
    )
    parser.add_argument(
        "--isoform-strategy",
        help="""See b-tea.cl_utils.
        Currently we only use 'merged'. By default, we use the 'unique' mapped reads,
        and no other option is implemented (e.g. stranded)""",
        default=None,
    )
    parser.add_argument(
        "--input-list",
        help="""A space-delimited list of sample names,
        each quoted separately. They must match the keys in the configuration file.
        Only these will be converted.""",
        nargs="*",
        type=str,
    )
    ...
    parser.add_argument(
        ...
        "must be taken into account if using [--chr-dict]",
        action="store_true",
    )
    parser.add_argument(
        "-d",
        "--chr-dict",
        help="""A dictionary mapping of
        sequence names found in the data to the sequence names, as in 'chrom.sizes'.
        The format is as follows: '{"key":"value"}'.""",
        type=json.loads,
    )
    parser.add_argument(
        "--overwrite",
    ...
    subparser = parser.add_subparsers(
        help="""Optional arguments if converting to bigWig.
        The program 'bedGraphToBigWig' must be available on the user's path, and can be"
        "downloaded from 'http://hgdownload.soe.ucsc.edu/admin/exe/'.""",
        dest="to_bigWig",
    )
    parser_to_bigWig = subparser.add_parser(
        "to-bigWig",
        help="""If given, will convert
        to bigWig.""",
    )
    parser_to_bigWig.add_argument(
        "chrSizes",
        help="""The 'chrom.sizes' file
    ...
    parser_to_bigWig.add_argument(
        "-k",
        "--keep-bed",
        help="If this flag is given, then " "the bedGraph file will not be deleted",
        action="store_true",
    )
    ...
    if os.path.exists(args.outloc):
        ...
        msg = "Invalid output path or wrong permission: {}. Quitting.".format(
            args.outloc
        )
        raise OSError(msg)
    ...
    for name in sorted(sample_names.keys()):
        ...
        lengths, offsets = ribo_utils.get_periodic_lengths_and_offsets(
            config, name, isoform_strategy=args.isoform_strategy, is_unique=is_unique
        )
        ...
        p_sites = (
            p_sites.groupby(p_sites.columns.tolist())
            .size()
            .reset_index()
        ...
        header = (
            ...
            sample_name_map[name], sample_name_map[name], display_mode, colour_positive
        )

        fn = "".join([name, note_str, iso_str, unique_str, ".sense.bedGraph"])
        bg_sense_output = os.path.join(str(args.outloc), fn)
        file_exists = (
            os.path.exists(bg_sense_output) and os.path.getsize(bg_sense_output) > 0
        )
        if args.overwrite or not file_exists:
            with open(bg_sense_output, "w") as f:
                f.write("{}\n".format(header))
            p_sites_positive.to_csv(
                bg_sense_output, index=False, header=False, sep="\t", mode="a"
            )
        else:
            msg = "The bedGraph file {} already exists. Skipping".format(
                bg_sense_output
            )
            logger.warning(msg)
        ...
        header = (
            ...
            sample_name_map[name], sample_name_map[name], display_mode, colour_negative
        )

        fn = "".join([name, note_str, iso_str, unique_str, ".anti-sense.bedGraph"])
        bg_antisense_output = os.path.join(str(args.outloc), fn)
        file_exists = (
            os.path.exists(bg_antisense_output)
            and os.path.getsize(bg_antisense_output) > 0
        )
        if args.overwrite or not file_exists:
            with open(bg_antisense_output, "w") as f:
                f.write("{}\n".format(header))
            p_sites_negative.to_csv(
                bg_antisense_output, index=False, header=False, sep="\t", mode="a"
            )
        else:
            msg = "The bedGraph file {} already exists. Skipping".format(
                bg_antisense_output
