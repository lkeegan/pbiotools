...
default_source = "ebi"
servers = {"ebi": "ftp.sra.ebi.ac.uk", "ncbi": "ftp-trace.ncbi.nih.gov"}
...
def extract_sra_file(row, args):
    ...
    if not os.path.exists(local_sra_file):
        ...
        if all(os.path.exists(f) for f in local_fastq_files):
            msg = "The .sra file has already been extracted: {}".format(local_sra_file)
            logger.info(msg)
            return
    ...
    cmd = (
        "fastq-dump {} {} {} --gzip --skip-technical --readids --dumpbase "
    ...
    if ret == 0:
        ...
        msg = "Problem extracting .sra file. It will be removed: {}".format(
            local_sra_file
        )
        logger.warning(msg)
...
def main():
    ...
    parser.add_argument(
        "-a",
        "--accession-field",
        help="The name of the column " "containing the SRR identifiers",
    ...
    parser.add_argument(
        "-p",
        "--paired-field",
        help="The name of the column " "indicating whether the sample is paired-end",
    ...
    parser.add_argument(
        "-s",
        "--source",
        help="The server from which the files " "will be downloaded",
    ...
    parser.add_argument(
        "--sep", help="The separator in the SRR file", default=default_sep
    )
    ...
    if args.use_slurm:
        msg = (
            "The --use-slurm option was given, so sbatch will now be used "
            "to submit to slurm."
        )
        logger.warning(msg)
