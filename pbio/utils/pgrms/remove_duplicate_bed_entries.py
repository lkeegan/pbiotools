...
def main():
    ...
    parser.add_argument("-o", "--out", help="The output bed(.gz) file", required=True)

    parser.add_argument(
        "--compress",
        help="If this flag is given, the output "
        "will be gzipped. The output filename *will not* be changed (so it "
        'should already end in ".gz").',
        action="store_true",
    )
    ...
    all_bed = [bed_utils.read_bed(b) for b in args.bed]
