...
def main():
    ...
    parser.add_argument(
        "-s",
        "--start",
        help="If specied, only alignments " "from this position will be in the output",
        default=default_start,
    )
    parser.add_argument(
        "-e",
        "--end",
        help="If specied, only alignments " "up to this position will be in the output",
        default=default_end,
    )
    ...
    parser.add_argument(
        "-k",
        "--keep-wig",
        help="If this flag is given, then " "the wiggle file will not be deleted",
        action="store_true",
    )
    ...
    parallel.apply_parallel_iter(
        args.bam, args.num_cpus, bam_to_wiggle, args, progress_bar=True
    )
...
def bam_to_wiggle(bam, args):
    ...
    if args.overwrite or not bigWig_exists:
        ...
        with closing(out_handle):
            chr_sizes, wig_valid = write_bam_track(
                bam, regions, out_handle, args.normalize
            )
        try:
...
def write_bam_track(bam_file, regions, out_handle, normalize):
    ...
    with indexed_bam(bam_file) as work_bam:
        total = (
            sum(1 for r in work_bam.fetch() if not r.is_unmapped) if normalize else None
        )
