...
def main():
    ...
    parser.add_argument(
        "read_counts", help="The counts csv file from " "count-aligned-reads"
    )
    parser.add_argument("out", help="The output image file")

    parser.add_argument(
        "--y-tick-frequency",
        help="The frequency of the ticks " "on the y-axis",
        type=int,
        default=default_y_tick_frequency,
    )

    parser.add_argument(
        "-t", "--title", help="The title for the plot", default=default_title
    )
    ...
    mpl_utils.create_stacked_bar_plot(
        ax,
        read_counts_np,
